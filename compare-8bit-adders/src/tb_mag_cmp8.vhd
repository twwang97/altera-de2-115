-- =============================================================================
-- File    : tb_mag_cmp8.vhd
-- Author  : twwang97
-- Date    : 2025-11-22
-- Purpose : Testbench for mag_cmp8 comparator. Sweeps all 8-bit vectors and
...
entity tb_mag_cmp8 is
-- testbench has no ports
end tb_mag_cmp8;

architecture beh of tb_mag_cmp8 is

    constant CASCADED_WIDTH  : natural := 8;
    constant MAX_TESTING_NUM : natural := 2 ** CASCADED_WIDTH - 1;
...
begin
    ...
    begin
        ...
        for ai in 0 to MAX_TESTING_NUM loop
            for bi in 0 to MAX_TESTING_NUM loop
                -- apply vectors
                A <= std_logic_vector(to_unsigned(ai, CASCADED_WIDTH));
                B <= std_logic_vector(to_unsigned(bi, CASCADED_WIDTH));

                -- allow combinational outputs to settle
                wait for 5 ns;

                -- compute expected results using unsigned comparisons
                if to_unsigned(ai,CASCADED_WIDTH) > to_unsigned(bi,CASCADED_WIDTH) then
                    exp_gt := '1'; exp_lt := '0'; exp_eq := '0';
                elsif to_unsigned(ai,CASCADED_WIDTH) < to_unsigned(bi,CASCADED_WIDTH) then
